...
""")
    ...
    # ✅ FIX: wrap in try-except (so errors show in UI)
    if submit_freight:
        try:
            input_data = {
            ...
            st.metric(
                ...
                value=f"${prediction.iloc[0]:,.2f}"   # ✅ FIX: safer indexing
            )

        except Exception as e:
            st.error(f"Error: {e}")   # 🔥 VERY IMPORTANT

# ==============================
# Invoice Flag Section
# ==============================
else:
...
""")
    ...
    if submit_flag:
        try:
            ...
            is_flagged = bool(flag_prediction.iloc[0])   # ✅ FIX
